...
Staravia::Staravia(const string & name, float x, float y, Player * player)
	:Monster(name, x, y)
{
	mPlayer = player;
}
...
void Staravia::Init()
{
	IMAGEMANAGER->LoadFromFile(L"Staravia", Resources(L"Staravia.bmp"), 320, 128, 10, 4, true);
	mImage = IMAGEMANAGER->FindImage(L"Staravia");
...
void Staravia::Update()
{
	...
	if (Input::GetInstance()->GetKeyDown('Y')) // 피격 테스트용임, 사용 끝나면 주석
	{
		mAttacked = true;
		mHp -= 20;
	}
