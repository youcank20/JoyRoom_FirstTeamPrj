...
Wobbuffet::Wobbuffet(const string & name, float x, float y, Player * player)
	:Monster(name, x, y)
{
	mPlayer = player;
}
...
void Wobbuffet::Init()
{
	IMAGEMANAGER->LoadFromFile(L"Wobbuffet", Resources(L"maja.bmp"), 261, 108, 9, 4, true);
	mImage = IMAGEMANAGER->FindImage(L"Wobbuffet");
	...
	mDownIdleAnimation->InitFrameByStartEnd(0, 0, 0, 0, false);
...
void Wobbuffet::Update()
{
	...
	if (Input::GetInstance()->GetKeyDown('R')) // 피격 테스트용임, 사용 끝나면 주석
	{
		mAttacked = true;
		mHp -= 20;
	}
	...
	if (mPrevIndex == 7 && mCurrentAnimation->GetNowFrameX() == 5)
		mAttackWait = true;
