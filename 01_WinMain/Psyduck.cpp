...
Psyduck::Psyduck(const string & name, float x, float y, Player * player)
	:Monster(name, x, y)
{
	mPlayer = player;
}
...
void Psyduck::Init()
{
	IMAGEMANAGER->LoadFromFile(L"Psyduck", Resources(L"gora.bmp"), 189, 124, 7, 4, true);
	mImage = IMAGEMANAGER->FindImage(L"Psyduck");
...
void Psyduck::Update()
{
	...
	if (Input::GetInstance()->GetKeyDown('T')) // 피격 테스트용임, 사용 끝나면 주석
	{
		mAttacked = true;
		mHp -= 20;
	}
	...
	if (mPrevIndex == 5 && mCurrentAnimation->GetNowFrameX() == 4)
		mAttackWait = true;
